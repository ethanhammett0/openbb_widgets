...
# Load .env fallback first (lowest priority)
load_dotenv(dotenv_path=Path(__file__).parent / ".env")

# Key resolution: env var → ODP user_settings.json → .env → empty
# pydantic-ai requires GEMINI_API_KEY; ODP stores it as 'gemini_api_key'
def _resolve_gemini_key() -> str:
    # 1. Already in environment (env var or .env)
    ...
    if settings.exists():
        try:
            creds = json.load(open(settings, encoding="utf-8")).get("credentials", {})
            key = creds.get("gemini_api_key") or creds.get("google_api_key", "")
            if key:
                return key
        except Exception:
            pass
...
if key:
    ...
    print(f"[INFO] Startup: GEMINI_API_KEY resolved ({key[:6]}...)")
else:
    print("[WARNING] Startup: GEMINI_API_KEY NOT FOUND — agent will fail to initialise")
